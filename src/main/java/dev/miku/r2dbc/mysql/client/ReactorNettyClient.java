...
final class ReactorNettyClient implements Client {
    ...
    @Override
    public Flux<ServerMessage> exchange(Flux<ClientMessage> requests, Predicate<ServerMessage> complete) {
        ...
        return Mono.<Flux<ServerMessage>>create(sink -> {
            ...
            boolean[] completed = new boolean[]{false}, firstError = new boolean[]{true};
            Flux<ServerMessage> responses = responseProcessor
    ...
    public Mono<Void> close() {
        return Mono.<Mono<Void>>create(sink -> {
            ...
            Mono<Void> exit = Mono.fromRunnable(() -> requestProcessor.onNext(ExitMessage.getInstance()));

            requestQueue.submit(RequestTask.wrap(sink, exit));
        }).flatMap(identity()).onErrorResume(e -> {
            logger.error(context.getMessage("Exit message sending failed, force closing"), e);
            return Mono.empty();
        }).then(forceClose());
    }
    ...
    public String toString() {
        return String.format("ReactorNettyClient{connectionId=%d, closing=%b}", context.getConnectionId(), this.closing.get());
    }
    ...
    private <T> Mono<T> resumeError(Throwable e) {
        ...
        logger.error(this.context.getMessage("Error: {}"), e.getMessage(), e);
        ...
        public void next(ServerMessage message) {
            if (INFO_ENABLED) {
                if (message instanceof WarningMessage) {
                    ...
                    logger.debug(ReactorNettyClient.this.context.getMessage("Response: {}"), message);
                }
            }
