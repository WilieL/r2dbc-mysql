...
final class RequestTask<T> {
    ...
    static <T> RequestTask<T> wrap(Flux<ClientMessage> messages, MonoSink<T> sink, T supplier) {
    ...
    private static final class DisposableFlux implements Disposable {
        ...
        private final Flux<ClientMessage> messages;

        private DisposableFlux(Flux<ClientMessage> messages) {
